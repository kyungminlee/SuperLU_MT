 ...
 * =======
 *
 * psdrive() is the main test program for the DOUBLE linear 
 * equation driver routines psgssv() and psgssvx().
 * 
 * The program is invoked by a shell script file -- pstest.csh.
 * The output from the tests are written into a file -- pstest.out.
 *
 * =====================================================================
 */
    float         *a, *a_save;
    ...
    fflush(stdout);
			...
			if ( prefact ) {	/* First time factor */
			    ...
			    /* Initialize options, reorder the matrix, 
			       obtain the column etree. 
		            ...
		            psgstrf_init(nprocs, fact, NOTRANS, NO, 
			    ...
			    psgstrf(&superlumt_options, &AC, perm_r, &L, &U, 
				    &Gstat, &info);
			...
			for (itran = 0; itran < NTRAN; ++itran) {
				...
				psgssv(nprocs, &A, perm_c, perm_r, &L, &U, 
				       &X, &info);
				...
				if ( info && info != izero ) {
				       ...
				       compute residual. */
                                    psgst01(m, n, &A, &L, &U, perm_c, perm_r,
					    &result[0]);
			    ...
			    /* Solve the system and compute the condition 
			       number and error bounds using psgssvx.   */
			    psgssvx(nprocs, &superlumt_options, &A, perm_c, 
				...
				if ( !info ) {
				    ...
				    sCopy_Dense_Matrix(m, nrhs, bsav, ldb,
						       wwork, ldb);
				    psgst02(trans, m, n, nrhs, &ASAV, solx,
					    ldx, wwork, ldb, &result[1]);

				    /* Check solution from generated exact
				       solution. */
				    psgst04(n, nrhs, solx, ldx, xact, ldx,
					    rcond, &result[2]);

				    /* Check the error bounds from iterative
				       refinement. */
				    psgst07(trans, n, nrhs, &ASAV, bsav, ldb,
